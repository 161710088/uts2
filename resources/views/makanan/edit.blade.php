	...
	<div class="container">
		<div class="col-md-12">
			<div class="panel panel-default">
			  <div class="panel-heading">Edit Data Makanan Khas 
			  	<div class="panel-title pull-right"><a href="{{ url()->previous() }}">Kembali</a>
			  	</div>
			  </div>
			  <div class="panel-body">
			  	<form action="{{ route('s.update',$h->id) }}" method="post" >
        			...
        			{{ csrf_field() }}
			  		...
			  		<div class="form-group {{ $errors->has('asal') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="asal" class="form-control" value="{{ $h->asal }}"  required>
			  			@if ($errors->has('asal'))
                            <span class="help-block">
			  		...
			  		<div class="form-group {{ $errors->has('harga') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="harga" class="form-control" value="{{ $h->harga }}"  required>
			  			@if ($errors->has('harga'))
                            <span class="help-block">
			  		...
			  		<div class="form-group {{ $errors->has('bahan_pokok') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="bahan_pokok" class="form-control" value="{{ $h->bahan_pokok }}"  required>
			  		...
			  		<div class="form-group {{ $errors->has('jenis_makanan') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="jenis_makanan" class="form-control" value="{{ $h->jenis_makanan }}"  required>
